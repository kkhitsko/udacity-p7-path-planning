...
bool checkLaneChangeSafe( double car_s, const size_t prev_size, int lane, const vector<vector<double> >& sensor_fusion ) {
    printf("Check line change %d safe\n", lane );
    bool is_lane_safe = true;

    for ( int i=0; i < sensor_fusion.size(); ++i ) {
        float d = sensor_fusion[i][6];
        int car_id = int(sensor_fusion[i][0]);
        ...
        if (event == "telemetry") {
          ...
          // j[1] is the data JSON object
        	...
        	// Main car's localization Data
          	...
          	/* Add 3 waypoints in 30m, 60m, 90m from starting points */
